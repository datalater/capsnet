...
+ MNIST 최첨단 성능 달성, CIFAR-10에서는 10% error지만 굿스타트
+ 더 적은 데이터로도 학습이 가능하다.
+ 포지션과 포즈 정보가 보존되는 equivariance를 제공한다.
+ equivariance가 보장되므로 이미지 세분화와 물체 인식에서 유망하다.
+ routing by agreement 알고리즘은 복잡한 장면에 적합하다.
+ 캡슐의 activation은 물체를 구성하는 부품과 잘 맵핑된다.
+ 아핀 변환에서 강건하다.
+ 활성 벡터가 해석이 가능하다. (rotation, skewness ...)


**단점**:

+ CIFAR-10에서 최첨단 성능에 도달하지 못했다.
+ ImageNet과 같은 더 큰 데이터에도 잘 적용되는지 테스트 되지 않았다.
+ routing-by-agreement 알고리즘이 3개의 inner loop가 있기 때문에 훈련 속도가 느리다.
+ 특정 타입을 인식하는 캡슐은 하나이므로 2개의 동일한 타입을 인식하지 못한다.
